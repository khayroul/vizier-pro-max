#!/usr/bin/env python3
"""Reconcile historical Hermes/OpenAI usage against Vizier metering evidence."""
...
def _cost_totals(
    ...
    *,
    ...
    where_sql, params = _time_clause(column="timestamp", start_ts=start_ts, end_ts=end_ts)
...
def _cost_risk_buckets(
    ...
    *,
    ...
    where_sql, params = _time_clause(column="timestamp", start_ts=start_ts, end_ts=end_ts)
    clauses = [where_sql[6:]] if where_sql else []
    provider_where = "provider_name IS NOT NULL"
    clauses_with_provider = [provider_where, *clauses]
    final_where = f"WHERE {' AND '.join(clauses_with_provider)}"
    row = conn.execute(
        f"""SELECT
                   COUNT(*) FILTER (WHERE session_id IS NULL OR TRIM(session_id) = '') AS missing_session_rows,
                   ...
                   COUNT(*) FILTER (WHERE deliverable_id IS NULL OR TRIM(deliverable_id) = '') AS missing_deliverable_rows,
                   ...
                   COUNT(*) FILTER (WHERE client_id IS NULL OR TRIM(client_id) = '') AS missing_client_rows,
...
def reconcile_usage(
    *,
    ...
    """Build a reconciliation report from Hermes/Vizier ledger evidence."""
    report: dict[str, Any] = {
        "db_path": str(db_path),
        "window": {
    ...
    with sqlite3.connect(str(db_path)) as conn:
        ...
        if _table_exists(conn, "cost_ledger"):
            all_cost = _cost_totals(conn, start_ts=start_ts, end_ts=end_ts)
            ...
            report["cost_ledger"] = {
                ...
                "openai_metered_rows": openai_cost,
                "hermes_metered_rows": hermes_metered,
                "lifecycle_only_rows": lifecycle_only,
        ...
        prompt_by_day = (
            _daily_prompt_totals(conn, start_ts=start_ts, end_ts=end_ts)
            if _table_exists(conn, "prompt_log")
            else {}
        )
        metered_by_day = (
    ...
    all_days = sorted(
        {
            *prompt_by_day.keys(),
            *metered_by_day.keys(),
            *hermes_metered_by_day.keys(),
            *openai_by_day.keys(),
            *lifecycle_by_day.keys(),
        }
    )
    daily_rows: list[dict[str, Any]] = []
    total_unattributed_hermes_tokens = 0
    total_unattributed_hermes_calls = 0
    for day in all_days:
        ...
        total_unattributed_hermes_tokens += unattributed_tokens
        total_unattributed_hermes_calls += unattributed_calls
        daily_rows.append(
            {
                "day": day,
    ...
    report["historical_gap_estimate"] = {
        "estimated_unattributed_hermes_tokens": total_unattributed_hermes_tokens,
        "estimated_unattributed_hermes_calls": total_unattributed_hermes_calls,
    ...
    if openai_total_tokens is not None or vizier_reported_tokens is not None:
        ...
        if openai_total_tokens is not None:
            ...
            comparison["remaining_gap_after_hermes_estimate"] = (
                openai_total_tokens
                - openai_metered_total
                - total_unattributed_hermes_tokens
            )
        if vizier_reported_tokens is not None:
            ...
            comparison["gap_metered_openai_minus_vizier_reported"] = (
                openai_metered_total - vizier_reported_tokens
            )
...
def render_text_report(report: dict[str, Any]) -> str:
    ...
    prompt_log = report.get("prompt_log", {})
    historical_gap = report.get("historical_gap_estimate", {})
    lines = [
        f"DB: {report['db_path']}",
        (
            "Window: "
            f"{report['window'].get('start_iso_utc') or 'beginning'}"
            " -> "
            f"{report['window'].get('end_iso_utc') or 'latest'}"
        ),
        "",
        "Cost ledger",
        f"  Provider-metered tokens: {cost_ledger.get('provider_metered_rows', {}).get('total_tokens', 0)}",
        f"  OpenAI-metered tokens: {cost_ledger.get('openai_metered_rows', {}).get('total_tokens', 0)}",
        f"  Hermes-metered tokens: {cost_ledger.get('hermes_metered_rows', {}).get('total_tokens', 0)}",
        f"  Lifecycle-only tokens: {cost_ledger.get('lifecycle_only_rows', {}).get('total_tokens', 0)}",
        "",
        "Prompt log",
        f"  Hermes prompt-log tokens: {prompt_log.get('total_tokens', 0)}",
        "",
        "Historical gap estimate",
        f"  Estimated unattributed Hermes tokens: {historical_gap.get('estimated_unattributed_hermes_tokens', 0)}",
        f"  Estimated unattributed Hermes calls: {historical_gap.get('estimated_unattributed_hermes_calls', 0)}",
    ]
    ...
    if top_days:
        lines.extend(["", "Top daily Hermes blind spots"])
        for row in top_days:
            lines.append(
                "  "
                f"{row['day']}: +{row['estimated_unattributed_hermes_tokens']} tokens "
                f"({row['estimated_unattributed_hermes_calls']} calls)"
            )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Path to the Hermes/Vizier SQLite state.db.",
    )
    parser.add_argument(
        "--start",
        help="Optional window start as unix timestamp or ISO8601 UTC/local string.",
    )
    parser.add_argument(
        "--end",
        help="Optional window end as unix timestamp or ISO8601 UTC/local string.",
    )
    parser.add_argument(
        "--openai-total-tokens",
        type=int,
        help="Optional OpenAI billing total for the same window.",
    )
    parser.add_argument(
        "--vizier-reported-tokens",
        type=int,
        help="Optional user-facing Vizier total for the same window.",
    )
    parser.add_argument(
        "--json",
        action="store_true",
