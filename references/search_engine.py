"""Reusable BM25 search helpers for local reference corpora."""
...
class BM25Index:
    ...
    def search(self, query: str, top_k: int = 5) -> list[dict[str, str | float]]:
        ...
        for index, score in ranked[:top_k]:
            row: dict[str, str | float] = {**self._documents[index]}
            row["score"] = round(score, 2)
            results.append(row)
        return results

    def _fallback(self, top_k: int) -> list[dict[str, str | float]]:
        """Return the first top_k documents when no terms match."""
        results: list[dict[str, str | float]] = []
