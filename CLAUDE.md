...
Pass `client_id` to poster pipelines to auto-apply brand colors, fonts, templates, and style hints:

```python
poster_generate.run(headline="...", body="...", client_id="dmb")
poster_batch.run(data_path="briefs.csv", client_id="dmb")
```

Available clients: `dmb`, `ar-rawdhah`, `rempah-tok-ma`

## Ultimate Port Closeout Boundary (2 April 2026)

- This closeout covers the Ultimate-derived poster branding/client config flow, document assembly tools, report and YouTube research tools, listening engine, and ads clone.
